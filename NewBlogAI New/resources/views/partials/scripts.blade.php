<script>
    ...
    function switchWorkspace(node) {
        ...
        if (node === 'prompts' && window.fetchPrompts) {
            window.fetchPrompts();
        }
        if (node === 'customers' && window.fetchCustomers) {
            window.fetchCustomers();
        }
        if (node === 'plans' && window.fetchPlans) {
            window.fetchPlans();
        }
    ...
    window.renderCustomers = function (customers) {
        ...
        if (customers.length === 0) {
            ...
            customers.forEach(customer => {
                ...
                // Subscription badge
                let subBadge = '';
                if (customer.subscription_status === 'active' || customer.subscription_status === 'trial') {
                    const planLabel = customer.plan_name ? ` · ${customer.plan_name}` : '';
                    subBadge = `<span class="ml-1 px-1.5 py-0.5 rounded bg-accent/15 text-accent border border-accent/30 text-[9px] font-mono">${customer.subscription_status}${planLabel}</span>`;
                } else if (customer.subscription_status) {
                    subBadge = `<span class="ml-1 px-1.5 py-0.5 rounded bg-muted/10 text-muted border border-border text-[9px] font-mono">${customer.subscription_status}</span>`;
                } else {
                    subBadge = `<span class="ml-1 px-1.5 py-0.5 rounded bg-warning/10 text-warning border border-warning/30 text-[9px] font-mono">no plan</span>`;
                }
                ...
                tr.innerHTML = `
                        ...
                        <td class="p-3">${statusHtml}${subBadge}</td>
                        <td class="p-3 text-muted">${customer.health_score || 100} / 100</td>
                        <td class="p-3 text-right pr-5">
                            ${subscriptionAction}
                            <button onclick="editCustomer('${customer.id}')" class="text-secondary hover:underline mr-3 text-xs">Edit</button>
                            <button onclick="deleteCustomer('${customer.id}')" class="text-error text-rose-400 hover:underline text-xs">Delete</button>
                        </td>
                    `;
                tbody.appendChild(tr);
            });
                ...
                try {
                    ...
                    if (logs.length === 0) {
                        ...
                        logs.forEach(l => {
                            ...
                            html += `
                                    <div class="p-2 border border-border rounded bg-background space-y-1">
        ...
        try {
            ...
            if (subscriptions.length === 0) {
                ...
                subscriptions.forEach(sub => {
                    const price = sub.plan ? (sub.billing_period === 'yearly' ? parseFloat(sub.plan.yearly_price || 0) : parseFloat(sub.plan.monthly_price || 0)) : 0;
    ...
    window.changePlanSubscription = async function (customerId, currentPlanId) {
        try {
            ...
            plans.forEach(plan => {
                ...
                planOptionsHtml += `<option value="${plan.id}" ${selected}>${plan.name} (₹${parseFloat(plan.monthly_price || 0).toFixed(2)}/mo)</option>`;
            });
            ...
            Swal.fire({
                ...
                html: `
                        <div class="text-left space-y-4">
                            <div>
                                ...
                                <select id="change-plan-select" class="w-full bg-background border border-border rounded-xl p-2.5 text-xs text-text font-mono focus:outline-none focus:border-accent">
                                    ${planOptionsHtml}
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-mono mb-1 text-muted uppercase">Billing Period</label>
                                <select id="change-plan-billing-period" class="w-full bg-background border border-border rounded-xl p-2.5 text-xs text-text font-mono focus:outline-none focus:border-accent">
                                    <option value="monthly" selected>Monthly</option>
                                    <option value="yearly">Yearly</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-mono mb-1 text-muted uppercase">Coupon Code (optional)</label>
                                <input type="text" id="change-plan-coupon-code" placeholder="Enter coupon..." class="w-full bg-background border border-border rounded-xl p-2.5 text-xs text-text font-mono focus:outline-none focus:border-accent">
                            </div>
                        </div>
                    `,
                showCancelButton: true,
                ...
                preConfirm: () => {
                    return {
                        ...
                        coupon_code: document.getElementById('change-plan-coupon-code').value || null
                    };
                }
    ...
    // Assign a brand-new subscription to a customer who has none
    window.assignSubscription = async function (customerId, companyName) {
        try {
            const res = await apiFetch('/api/v1/plans');
            if (!res.ok) throw new Error('Could not load pricing plans.');
            const result = await res.json();
            ...
            if (!plans.length) {
                return showError('No Plans', 'Create at least one plan before assigning a subscription.');
            }

            let planOptionsHtml = '';
            plans.forEach(plan => {
                planOptionsHtml += `<option value="${plan.id}">${plan.name} (₹${parseFloat(plan.monthly_price || 0).toFixed(2)}/mo)</option>`;
            });

            Swal.fire({
                title: `Assign Subscription`,
                html: `
                        <p class="text-xs text-muted mb-4">Customer: <strong>${companyName || customerId}</strong></p>
                        <div class="text-left space-y-4">
                            <div>
                                <label class="block text-xs font-mono mb-1 text-muted uppercase">Plan</label>
                                <select id="assign-plan-select" class="w-full bg-background border border-border rounded-xl p-2.5 text-xs text-text font-mono focus:outline-none focus:border-accent">
                                    ${planOptionsHtml}
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-mono mb-1 text-muted uppercase">Billing Period</label>
                                <select id="assign-billing-period" class="w-full bg-background border border-border rounded-xl p-2.5 text-xs text-text font-mono focus:outline-none focus:border-accent">
                                    <option value="monthly" selected>Monthly</option>
                                    <option value="yearly">Yearly</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-mono mb-1 text-muted uppercase">Status</label>
                                <select id="assign-status" class="w-full bg-background border border-border rounded-xl p-2.5 text-xs text-text font-mono focus:outline-none focus:border-accent">
                            ...
                            <div>
                                <label class="block text-xs font-mono mb-1 text-muted uppercase">Coupon Code (optional)</label>
                                <input type="text" id="assign-coupon-code" placeholder="e.g. LAUNCH50" class="w-full bg-background border border-border rounded-xl p-2.5 text-xs text-text font-mono focus:outline-none focus:border-accent">
                            </div>
                        </div>
                    `,
                showCancelButton: true,
                ...
                preConfirm: () => {
                    ...
                    if (!planId) { Swal.showValidationMessage('Please select a plan.'); return false; }
                    return {
                        plan_id: planId,
                        billing_period: document.getElementById('assign-billing-period').value,
                        payment_token: 'manual-assign',
                        status: document.getElementById('assign-status').value,
                        coupon_code: document.getElementById('assign-coupon-code').value || null,
                    };
                }
            }).then(async (dialogResult) => {
                if (!dialogResult.isConfirmed) return;
                await apiRequest(`/api/v1/customers/${customerId}/subscription`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dialogResult.value)
                }, {
                    successTitle: 'Subscription Assigned',
                    successMessage: `Plan assigned to ${companyName || 'customer'} successfully.`,
                    defaultErrorMessage: 'Could not assign subscription.',
                    onSuccess: async () => { await fetchBillingLedger(); }
                });
            });
        } catch (err) {
            console.error('Error in assignSubscription:', err);
            showError('Error', err.message || 'Failed to load plans.');
        }
    };
    ...
    window.fetchSystemSettings = async function () {
        try {
            ...
            if (res.ok) {
                ...
                // Populate Primary Model Routing dropdown dynamically from AI Providers
                const modelSelect = document.getElementById('setting-primary-model');
                if (modelSelect) {
                    try {
                        ...
                        if (provRes.ok) {
                            ...
                            modelSelect.innerHTML = providers
                                ...
                                .map(p => `<option value="${p.provider_key}" ${p.is_default ? 'selected' : ''}>${p.name} (${p.default_model || ''})</option>`)
                                .join('');
                        }
                    } catch (e) {
                        console.error("Error populating primary model select:", e);
                    }
                }
            }
        } catch (err) {
    ...
    window.fetchTopics = async function () {
        ...
        try {
            ...
            if (!res.ok) return;
            const result = await res.json();
            allTopicsCache = result.data || result;

            // Populate category filter dropdown dynamically
            const categorySelect = document.getElementById('topic-filter-category');
            if (categorySelect) {
                const currentVal = categorySelect.value;
                const categories = [...new Set(allTopicsCache.map(t => t.category).filter(Boolean))];
    ...
    window.populateTopicPromptOptions = async function () {
        const promptSelect = document.getElementById('topic-prompt-id');
        ...
        try {
            ...
            const result = await res.json();
            const prompts = result.data || result;
            promptSelect.innerHTML = '<option value="">None (Use default prompt)</option>' +
                prompts.map(p => `<option value="${p.id}">${p.name}</option>`).join('');
        } catch (e) {
            console.error("Error loading prompts for topic dropdown:", e);
        }
    };
    ...
    window.renderTopics = function () {
        ...
        const categoryFilter = document.getElementById('topic-filter-category')?.value || '';
        const filtered = allTopicsCache.filter(t => !categoryFilter || t.category === categoryFilter);
        ...
        if (filtered.length === 0) {
            ...
            filtered.forEach(topic => {
                ...
                let statusClass = "bg-success/20 text-success border-success/30";
                if (topic.status === 'paused' || topic.status === 'inactive') {
                    statusClass = "bg-warning/20 text-warning border-warning/30";
                } else if (topic.status === 'draft') {
                    statusClass = "bg-white/10 text-muted border-white/20";
                }
                ...
                tr.innerHTML = `
                        ...
                        <td class="p-3 text-muted">${topic.category || 'General'}</td>
                        <td class="p-3 text-muted">${topic.language || 'English'}</td>
                        <td class="p-3 text-muted uppercase font-mono text-[10px]">${topic.priority || 'medium'}</td>
                        <td class="p-3 text-muted font-mono text-[10px]">${topic.generation_frequency || 'daily'}</td>
                        <td class="p-3"><span class="px-2 py-0.5 rounded border text-[9px] ${statusClass}">${topic.status || 'active'}</span></td>
                        <td class="p-3 text-right pr-5">
                            <button onclick="openTopicEditModal(${topic.id})" class="text-secondary hover:underline mr-3 text-xs">Edit</button>
                            <button onclick="deleteTopic(${topic.id})" class="text-rose-400 hover:underline text-xs">Delete</button>
                        </td>
                    `;
                tbody.appendChild(tr);
            });
    ...
    window.openTopicAddModal = function () {
        ...
        document.getElementById('topic-tags').value = '';
        document.getElementById('topic-modal-title').innerText = 'Add Content Topic';
        window.populateTopicPromptOptions();
        modal.classList.add('active');
    };

    window.openTopicEditModal = async function (id) {
        ...
        await window.populateTopicPromptOptions();
        ...
        document.getElementById('topic-frequency').value = topic.generation_frequency || 'daily';
        document.getElementById('topic-prompt-id').value = topic.prompt_id || '';
        document.getElementById('topic-tags').value = Array.isArray(topic.tags) ? topic.tags.join(', ') : (topic.tags || '');
        document.getElementById('topic-modal-title').innerText = 'Edit Content Topic';
    ...
    window.saveTopic = async function (event) {
        ...
        await apiRequest(url, {
            ...
            successTitle: id ? "Topic Updated" : "Topic Created",
            successMessage: id ? "Topic details saved successfully." : "New content topic created successfully.",
            onSuccess: async () => {
                window.closeTopicModal();
                await window.fetchTopics();
            }
        });
    };
    ...
    window.deleteTopic = async function (id) {
        showConfirmation(
            ...
            async () => {
                await apiRequest(`/api/v1/topics/${id}`, {
                    method: 'DELETE'
                }, {
                    successTitle: "Topic Deleted",
                    successMessage: "Topic has been deleted successfully.",
                    onSuccess: async () => {
                        await window.fetchTopics();
                    }
                });
    ...
    window.fetchPrompts = async function () {
        const listContainer = document.getElementById('prompt-templates-list');
        if (!listContainer) return;
        ...
        try {
            ...
            if (!res.ok) return;
            const result = await res.json();
            allPromptsCache = result.data || result;

            listContainer.innerHTML = '';
            if (allPromptsCache.length === 0) {
                listContainer.innerHTML = `
                        <div class="p-6 text-center text-muted font-mono text-xs">
                            <span class="material-symbols-outlined text-3xl mb-2 text-muted block">psychology</span>
                            No prompts found.<br>Click "Create Template" to add your first prompt.
                        </div>
                    `;
                window.openNewPromptTemplate();
            } else {
                allPromptsCache.forEach(prompt => {
                    const activeId = document.getElementById('prompt-active-id')?.value;
                    const isSelected = String(prompt.id) === String(activeId);

                    const div = document.createElement('div');
                    div.className = `p-3 rounded-xl cursor-pointer transition group relative prompt-list-item ${isSelected ? 'bg-white/5 border border-accent' : 'bg-transparent border border-border hover:bg-white/5'}`;
                    div.id = `prompt-item-${prompt.id}`;
                    div.onclick = () => window.selectPromptTemplate(prompt.id);

                    let statusColor = prompt.status === 'active' ? 'text-success' : 'text-warning';

                    div.innerHTML = `
                            <div class="flex justify-between items-center mb-1">
                                <p class="text-xs font-semibold text-text">${escapeJsString(prompt.name)}</p>
                ...
                // Auto-select first prompt if none currently selected
                const activeId = document.getElementById('prompt-active-id')?.value;
                if (!activeId && allPromptsCache.length > 0) {
                    window.selectPromptTemplate(allPromptsCache[0].id);
                }
            }
        } catch (err) {
    ...
    window.selectPromptTemplate = function (id) {
        const prompt = allPromptsCache.find(p => String(p.id) === String(id));
        if (!prompt) return;
        ...
        document.querySelectorAll('.prompt-list-item').forEach(el => {
            if (el.id === `prompt-item-${prompt.id}`) {
                el.classList.add('bg-white/5', 'border', 'border-accent');
                el.classList.remove('bg-transparent', 'border-border');
            } else {
                el.classList.remove('bg-white/5', 'border-accent');
                el.classList.add('bg-transparent', 'border', 'border-border');
            }
        });
    };
    ...
    window.openNewPromptTemplate = function () {
        ...
        document.getElementById('prompt-edit-status').value = 'active';
        document.getElementById('prompt-editor-textarea').value = '';
        document.getElementById('prompt-editor-id').innerText = 'New Prompt Template';
    ...
    window.saveActivePrompt = async function () {
        ...
        if (!name.trim()) {
            showToast("Name Required", "Please specify a prompt template name.", "warning");
            return;
        }
        if (!promptText.trim()) {
            showToast("Instructions Required", "Please enter prompt template instructions.", "warning");
            return;
        }
    ...
    window.deleteActivePrompt = async function () {
        ...
        showConfirmation(
            ...
            async () => {
                await apiRequest(`/api/v1/prompts/${id}`, {
                    method: 'DELETE'
                }, {
                    successTitle: "Template Deleted",
                    successMessage: "Prompt template has been removed.",
                    onSuccess: async () => {
                        window.openNewPromptTemplate();
                        await window.fetchPrompts();
                    }
    ...
    window.insertPromptVar = function (varName) {
    ...
    window.switchPromptSubTab = function (tab) {
        const editorTab = document.getElementById('prompt-tab-editor');
        const testerTab = document.getElementById('prompt-tab-tester');
        const editorPane = document.getElementById('prompt-pane-editor');
        const testerPane = document.getElementById('prompt-pane-tester');

        if (!editorPane || !testerPane) return;

        if (tab === 'editor') {
            editorPane.classList.remove('hidden');
            testerPane.classList.add('hidden');
            editorTab.classList.add('text-accent', 'border-accent');
            editorTab.classList.remove('text-muted', 'border-transparent');
            testerTab.classList.remove('text-accent', 'border-accent');
            testerTab.classList.add('text-muted', 'border-transparent');
        } else {
            editorPane.classList.add('hidden');
            testerPane.classList.remove('hidden');
            testerTab.classList.add('text-accent', 'border-accent');
            testerTab.classList.remove('text-muted', 'border-transparent');
            editorTab.classList.remove('text-accent', 'border-accent');
            editorTab.classList.add('text-muted', 'border-transparent');
        }
    };

    window.runPromptTestSimulation = async function () {
        const activeId = document.getElementById('prompt-active-id').value;
        ...
        try {
            if (activeId) {
                const res = await apiFetch(`/api/v1/prompts/${activeId}/test`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                ...
                if (res.ok) {
                    const data = await res.json();
                    outputWin.innerText = data.text || data.compiled_prompt || "Dry-run completed with empty output.";
                    return;
                }
            }
            ...
            let compiled = promptText
                ...
                .replaceAll('@{{website}}', 'https://example.com')
                .replaceAll('@{{date}}', new Date().toLocaleDateString());

            outputWin.innerText = "[[ DRY-RUN COMPILED PROMPT PREVIEW ]]\n\n" + compiled;
        } catch (e) {
            outputWin.innerText = "Error running dry-run test: " + e.message;
        }
    };
        ...
        try {
            ...
            if (plans.length === 0) {
                ...
                plans.forEach(plan => {
                    ...
                    // Feature labels
                    let featuresList = [];
                    if (plan.analytics_access) featuresList.push('Analytics');
                    if (plan.priority_support) featuresList.push('Priority Support');
                    if (plan.feature_flags) {
                        if (plan.feature_flags.seo_optimizer) featuresList.push('SEO');
                        if (plan.feature_flags.localization) featuresList.push('Localization');
                    }
                    const featuresHtml = featuresList.map(f => `<span class="px-1.5 py-0.5 rounded bg-background border border-border text-[9px] text-muted">${f}</span>`).join(' ') || '<span class="text-muted text-[10px]">—</span>';

                    // AI providers
                    const providers = (plan.ai_providers_available || []).map(p => p.toUpperCase()).join(', ') || 'NONE';

                    tr.innerHTML = `
                            <td class="p-3 pl-5 text-text font-medium">${plan.name}</td>
                            <td class="p-3 text-accent font-bold">₹${parseFloat(plan.monthly_price).toFixed(2)}/mo<br><span class="text-muted text-[10px] font-normal">₹${parseFloat(plan.yearly_price).toFixed(2)}/yr</span></td>
                            <td class="p-3 text-muted">${plan.max_wordpress_sites} sites<br>${plan.max_topics} topics</td>
                            <td class="p-3 text-muted">${plan.storage_limit} MB storage<br>${plan.monthly_generation_limit} MTD Gen quota</td>
                            <td class="p-3">${featuresHtml}<div class="text-[9px] text-muted mt-1 font-mono">Providers: ${providers}</div></td>
                            <td class="p-3"><span class="px-2 py-0.5 rounded border text-[9px] ${statusClass}">${plan.status}</span></td>
                            <td class="p-3 text-right pr-5">
                                <button onclick="openPlanEditModal(${plan.id})" class="text-secondary hover:underline mr-3 text-xs">Edit</button>
                                <button onclick="deletePlan(${plan.id})" class="text-error text-rose-400 hover:underline text-xs">Delete</button>
                            </td>
                        `;
                    tbody.appendChild(tr);
                });
    ...
    window.openPlanAddModal = function () {
        const modal = document.getElementById('plan-modal');
        if (!modal) return;

        document.getElementById('plan-modal-title').innerText = "Define Subscription Plan";
        document.getElementById('plan-id').value = '';
        document.getElementById('plan-name').value = '';
        document.getElementById('plan-status').value = 'active';
        document.getElementById('plan-monthly-price').value = '79';
        document.getElementById('plan-yearly-price').value = '790';
        document.getElementById('plan-max-wordpress-sites').value = '10';
        document.getElementById('plan-max-topics').value = '50';
        document.getElementById('plan-publishing-schedule-limit').value = '20';
        document.getElementById('plan-max-articles-per-day').value = '50';
        document.getElementById('plan-monthly-generation-limit').value = '500';
        document.getElementById('plan-minimum-publishing-frequency').value = 'daily';
        document.getElementById('plan-prompt-templates-allowed').value = '20';
        document.getElementById('plan-api-keys-allowed').value = '5';
        document.getElementById('plan-storage-limit').value = '5120';

        // Checkboxes
        document.getElementById('plan-analytics-access').checked = true;
        document.getElementById('plan-priority-support').checked = true;
        document.getElementById('plan-ff-seo').checked = true;
        document.getElementById('plan-ff-loc').checked = false;

        // AI Providers list checkboxes
        document.querySelectorAll('input[name="plan-ai-providers"]').forEach(cb => {
    ...
    window.openPlanEditModal = async function (id) {
        ...
        try {
            showLoading("Fetching Plan Settings...");
            const res = await apiFetch(`/api/v1/plans/${id}`);
            if (!res.ok) throw new Error("Could not retrieve plan details.");
            const result = await res.json();
            const plan = result.data || result;
            Swal.close();

            document.getElementById('plan-modal-title').innerText = "Edit Pricing Plan Options";
            ...
            document.getElementById('plan-yearly-price').value = plan.yearly_price;
            document.getElementById('plan-max-wordpress-sites').value = plan.max_wordpress_sites;
            document.getElementById('plan-max-topics').value = plan.max_topics;
            document.getElementById('plan-publishing-schedule-limit').value = plan.publishing_schedule_limit;
            document.getElementById('plan-max-articles-per-day').value = plan.max_articles_per_day;
            document.getElementById('plan-monthly-generation-limit').value = plan.monthly_generation_limit || '100';
            document.getElementById('plan-minimum-publishing-frequency').value = plan.minimum_publishing_frequency || 'daily';
            document.getElementById('plan-prompt-templates-allowed').value = plan.prompt_templates_allowed;
            document.getElementById('plan-api-keys-allowed').value = plan.api_keys_allowed;
            document.getElementById('plan-storage-limit').value = plan.storage_limit;

            // Checkboxes
            document.getElementById('plan-analytics-access').checked = !!plan.analytics_access;
            document.getElementById('plan-priority-support').checked = !!plan.priority_support;

            const ff = plan.feature_flags || {};
            document.getElementById('plan-ff-seo').checked = !!ff.seo_optimizer;
            document.getElementById('plan-ff-loc').checked = !!ff.localization;

            // AI Providers
            const providers = plan.ai_providers_available || [];
            document.querySelectorAll('input[name="plan-ai-providers"]').forEach(cb => {
            ...
            showError("Error", err.message || "Failed to load plan settings.");
        }
    };
    ...
    window.savePlan = async function (event) {
        ...
        const monthly_price = parseFloat(document.getElementById('plan-monthly-price').value);
        const yearly_price = parseFloat(document.getElementById('plan-yearly-price').value);
        const max_wordpress_sites = parseInt(document.getElementById('plan-max-wordpress-sites').value);
        const max_topics = parseInt(document.getElementById('plan-max-topics').value);
        const publishing_schedule_limit = parseInt(document.getElementById('plan-publishing-schedule-limit').value);
        const max_articles_per_day = parseInt(document.getElementById('plan-max-articles-per-day').value);
        const monthly_generation_limit = parseInt(document.getElementById('plan-monthly-generation-limit').value);
        const minimum_publishing_frequency = document.getElementById('plan-minimum-publishing-frequency').value;
        const prompt_templates_allowed = parseInt(document.getElementById('plan-prompt-templates-allowed').value);
        const api_keys_allowed = parseInt(document.getElementById('plan-api-keys-allowed').value);
        const storage_limit = parseInt(document.getElementById('plan-storage-limit').value);

        const analytics_access = document.getElementById('plan-analytics-access').checked;
        const priority_support = document.getElementById('plan-priority-support').checked;

        const feature_flags = {
            seo_optimizer: document.getElementById('plan-ff-seo').checked,
            localization: document.getElementById('plan-ff-loc').checked
        };

        const ai_providers_available = [];
        document.querySelectorAll('input[name="plan-ai-providers"]:checked').forEach(cb => {
            ai_providers_available.push(cb.value);
        });
        ...
        const url = id ? `/api/v1/plans/${id}` : '/api/v1/plans';
        const method = id ? 'PUT' : 'POST';
        const submitBtn = event.target.querySelector('button[type="submit"]');
        ...
        await apiRequest(url, {
            ...
            successTitle: "Plan Options Saved",
            successMessage: id ? "Pricing plan details updated successfully." : "Pricing plan defined successfully.",
            defaultErrorMessage: "Failed to persist subscription plan config.",
            submitBtn: submitBtn,
            onSuccess: async () => {
                closePlanModal();
                await fetchPlans();
    ...
    window.deletePlan = async function (id) {
        showConfirmation(
            ...
            "Are you sure you want to completely remove this plan? Existing subscriptions mapped to it will not be deleted but they will lose limits references.",
            async () => {
                await apiRequest(`/api/v1/plans/${id}`, { method: 'DELETE' }, {
                    successTitle: "Plan Removed",
                    successMessage: "Subscription plan soft-deleted successfully.",
                    defaultErrorMessage: "Could not remove plan from configuration database.",
                    onSuccess: async () => {
                        await fetchPlans();
                    }
                });
