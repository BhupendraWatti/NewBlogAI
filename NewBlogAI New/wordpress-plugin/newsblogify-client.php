...
function run_newsblogify_client() {
    ...
    NewsBlogify\REST_Controller::register();

    // Initialize WP Cron scheduler
    NewsBlogify\Cron::get_instance();

    // Initialize Admin Dashboard UI and Setup Wizard
    if ( is_admin() ) {
        NewsBlogify\Admin::get_instance();
    }
}

// Activation hook
register_activation_hook( __FILE__, function() {
    NewsBlogify\Cron::get_instance()->schedule_events();
    NewsBlogify\Logger::get_instance()->log( 'info', 'Plugin activated.' );
} );

// Deactivation hook
register_deactivation_hook( __FILE__, function() {
    NewsBlogify\Cron::get_instance()->clear_events();
    NewsBlogify\Logger::get_instance()->log( 'info', 'Plugin deactivated.' );
} );
